...
Trained hierarchy & 293 & 25.68 & 0.2142 & 150 \\

Identical untrained prior & 375 & 30.72 & 0.1937 & 105 \\

Greedy & 345 & 25.62 & 0.1950 & 105 \\

Outer only & 325 & 28.20 & 0.2123 & 150 \\

Inner only & 345 & 24.82 & 0.2003 & 120 \\

Uniform cost & 15 & 0.00 & 0.0252 & 0 \\

Audit only & 15 & 0.00 & 0.0238 & 0 \\
...
The trained hierarchy has -5.04 percentage points less mean normalized savings than the untrained prior; the paired target-cluster 95\% interval is [-10.06, -0.22]. This is negative transfer, not evidence of a learned speed or resource advantage. The 121 hierarchy runs with strict incumbent improvement demonstrate working anytime synthesis, but the untrained control also improves incumbents. Correct output circuits and improvement during search therefore do not establish a benefit from training.

The mean final-model training cost is 4.816 CPU seconds (4.816 wall seconds), across five models of 184 episodes each. At 1,000 uses this contributes 0.0048 CPU seconds per target. These figures exclude hyperparameter selection, ablation retraining, and the later shrinkage-selection experiment; all are separate artifact records. A small training bill does not compensate for worse discovery performance.
...
Adapted GraySynth & 72 & 33.61 & 0.0021 \\

Parity star & 75 & 0.00 & 0.0026 \\

Rank-partition reference & 0 & 0.00 & 0.0062 \\
...
Retrained without budget features & 110 & 25.21 & 32 \\

Retrained without workspace features & 86 & 20.65 & 32 \\
...
The bounded panel scores at most 32 candidates, compared with an observed maximum of 647 under full-frontier scoring. With identical trained weights, the bounded version improves normalized savings by 3.17 percentage points in the matched secondary contrast (descriptive 95\% interval [0.16, 7.05]). This supports limiting ranking overhead within this implementation; it does not establish that learning itself is beneficial. The secondary timings are not additional independent replications of the primary contrast.
...
Raw trained hierarchy & 508 & 26.40 \\
...
The raw hierarchy remains worse than the prior: -1.34 percentage points (95\% interval [-5.00, 2.23]). The guarded-minus-prior effect is 0.000 percentage points, interval [0.000, 0.000]. Because their scores are identical at $\beta=0$, small wall-budget differences reflect timing and termination variability, not learned generalization. This amendment prevents unsupported deployment of a degraded learned controller; it does not rescue the positive learning hypothesis.
...
Post-campaign auditing closes 11 of the 25 primary CNOT gaps; the other 14 remain certified upper bounds. No audit result repairs a failed discovery record. The complete evidence verifier replays 654 unique native witness/contract pairs across 5,501 recorded outcomes, checks 26 closed covers and 120 rank-bound records, and verifies the coherent evaluator reference. The 5,501 outcomes include calibration, validation, constructive controls and the amendment; they are not 5,501 independent target problems.
