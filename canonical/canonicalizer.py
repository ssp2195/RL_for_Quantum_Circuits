...
from algebra.pauli_rotation import normalize_clifford_semantics
...
class Canonicalizer:
    ...
    ``absorb_clifford_angles=False`` disables only the emergent-Clifford pass
    for controlled tiny-instance ablations; production search should retain
    the default sound reduction.
    """

    SCHEMA = "clifford-pauli-rotation-v2"
    RAW_WITNESS_SCHEMA = "raw-authoritative-dag-witness-v1"
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(phase_sensitive, bool):
            raise TypeError("phase_sensitive must be a bool")
        if not isinstance(absorb_clifford_angles, bool):
            raise TypeError("absorb_clifford_angles must be a bool")
        if normalization_mode not in {"enhanced", "raw_witness"}:
            raise ValueError("normalization_mode must be 'enhanced' or 'raw_witness'")
    ...
    def semantic_key(
        ...
        *,
        ...
        if self.normalization_mode == "raw_witness":
            # Ablation-only, deliberately incomplete identity: exact DAG words
            # are equal only to themselves.  This performs no commuting
            # reorder, cross-position fusion, or Clifford-angle absorption and
            # therefore can increase search but can never falsely merge two
            # different witnesses.
            gates = tuple(
            ...
            return (
                self.RAW_WITNESS_SCHEMA,
                state.dag.num_qubits,
                gates,
                self._continuation_interface(state),
                ("literal-phase-requested", bool(include_global_phase)),
            )
        ...
        if frame is None:
            ...
            normalized_rotations = tuple(getattr(state, "rotations", ()))
            normalized_phase = int(getattr(state, "global_phase_eighths", 0)) % 16
        else:
            normalized_frame, normalized_rotations, normalized_phase = (
                normalize_clifford_semantics(
                    frame,
                    getattr(state, "rotations", ()),
                    getattr(state, "global_phase_eighths", 0),
                    absorb_clifford_angles=self.absorb_clifford_angles,
                )
            )
            if hasattr(normalized_frame, "canonical_payload"):
                frame_payload = tuple(normalized_frame.canonical_payload())
            elif hasattr(normalized_frame, "stable_payload"):
                frame_payload = tuple(normalized_frame.stable_payload())
        ...
        key: Tuple[Any, ...] = (
            ...
            ("absorb-clifford-angles", self.absorb_clifford_angles),
            state.dag.num_qubits,
            frame_payload,
            rotations,
            interface,
        ...
        if include_global_phase:
            ...
            frame_lift = (
                tuple(normalized_frame.phase_sensitive_payload())
                if normalized_frame is not None
                and hasattr(normalized_frame, "phase_sensitive_payload")
                else ()
            )
