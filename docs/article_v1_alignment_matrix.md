...
| Eq. 129: phase-quotiented trace/Frobenius discrepancy and independent DAG replay | `certification/article_v1.py` (`ArticleV1CertificationEngine`, `article_v1_certification_diagnostics`) and binding in `experiments/article_v1_runner.py` | `tests/article_v1/test_certification.py`; legacy certifier regressions in `tests/test_certification_simulator.py` | Implemented and focused/mini-tested | Acceptance uses \(\Delta_\phi=\sqrt{1-|\mathrm{Tr}(V^\dagger U)|/d}\) and default `tau_cert=1e-9`; it is distinct from feature distance \(d_{\mathrm{tar}}=1-c_\phi^2\) and corpus `tau_identity`. The final mini artifact binds this engine across all seven scheduler labels and independently certifies the known reachable target under FIFO without witness fallback. |
